...
<div id="Content_Content_ctl02_InputWrapControl" class="InputWrap Required HideError">
	...
	<div class="InputParent">
		...
		<!-- Options -->
			...
			<img src="https://natgeo.imgix.net/misc/newsletter.jpg?auto=compress,format&w=1920&h=400&fit=crop" alt="" />
		</div>
...
<div id="Content_Content_ctl04_Link_InputWrapControl" class="InputWrap Required">
	...
	<script type="text/javascript">
		$(function () {
			if ($('.PlasticsPledge').length) {
				plasticsPrevScrollTop = 0;
				$(window).scroll(function () {
					if ($(window).scrollTop() + $(window).height() < $('.Newsletter').offset().top) {
						$('.PlasticsPledge').addClass('Visible');
					} else {
						$('.PlasticsPledge').removeClass('Visible');
					}
					if (Math.abs($(window).scrollTop() - plasticsPrevScrollTop) > 100) {
						plasticsPrevScrollTop = $(window).scrollTop();
					}
				});
			}
		});
